...
//! manager directly.
//! Bundles can also consume resources through their own RON `resources` map.
//! Declare inputs with `resources!`, set [`ResourceBundle::INPUT_NAMES`] to the
//! generated [`ResourceBindings::NAMES`], and call [`BundleContext::inputs`] during
//! `build`. The runtime macro orders bundle construction per mission and compiles
//! bindings into static keys. Existing bundles have no inputs by default.
...
pub trait ResourceBindings<'r>: Sized {
    ...
    /// Input names in declaration order, generated by `resources!`.
    const NAMES: &'static [&'static str] = &[];
    ...
    fn from_keys(_manager: &'r mut ResourceManager, _keys: &[ResourceKey]) -> CuResult<Self> {
        Err(CuError::from(
            "Resource bindings do not support bundle inputs",
        ))
    }
...
impl<'r> ResourceBindings<'r> for () {
    ...
    fn from_keys(_manager: &'r mut ResourceManager, keys: &[ResourceKey]) -> CuResult<Self> {
        if !keys.is_empty() {
            return Err(CuError::from("Unexpected resource inputs"));
        }
        Ok(())
    }
...
impl<B: ResourceBundleDecl> BundleContext<B> {
    ...
    /// Attach statically generated input keys, in `INPUT_NAMES` order.
    pub const fn with_input_keys(mut self, keys: &'static [ResourceKey]) -> Self {
        self.input_keys = keys;
        self
    }

    /// Resolve typed construction inputs. Owned inputs move into the new resource;
    /// shared inputs clone a handle. Borrowed inputs cannot outlive the manager borrow.
    pub fn inputs<'r, R: ResourceBindings<'r>>(
        &self,
        manager: &'r mut ResourceManager,
    ) -> CuResult<R>
    where
        B: ResourceBundle,
    {
        if B::INPUT_NAMES != R::NAMES || self.input_keys.len() != R::NAMES.len() {
            return Err(CuError::from("Resource bundle input declaration mismatch"));
        }
        R::from_keys(manager, self.input_keys)
    }
}

/// Order and validate bundle inputs during constant evaluation, without runtime
/// name lookup or allocations. Missing, duplicate and unknown bindings are errors.
#[doc(hidden)]
pub const fn resource_input_keys<const N: usize>(
    names: &[&str],
    entries: &[(&str, ResourceKey)],
) -> [ResourceKey; N] {
    assert!(
        names.len() == N && entries.len() == N,
        "missing resource bundle inputs"
    );
    let mut keys = [ResourceKey::new(BundleIndex::new(0), 0); N];
    let mut seen = [false; N];
    let mut entry = 0;
    while entry < entries.len() {
        let mut index = 0;
        while index < N && !str_eq(names[index], entries[entry].0) {
            index += 1;
        }
        assert!(index < N, "unknown resource bundle input");
        assert!(!seen[index], "duplicate resource bundle input");
        keys[index] = entries[entry].1;
        seen[index] = true;
        entry += 1;
    }
    keys
