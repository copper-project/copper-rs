...
mod tests {
    ...
    mod payload_clearing {
        ...
        #[test]
        fn test_clear_default_tuple_handle_payload() {
        ...
        #[test]
        fn test_reuse_preserves_tuple_handle_payload() {
            let mut lists = CuListsManager::<cumsgs::CuStampedDataSet, 1>::new();
            let list = lists.create().unwrap();
            list.msgs
                .0
                .0
                .set_payload((true, CuHandle::new_detached(vec![1, 2, 3])));
            ...
            let (flag, handle) = reused.msgs.0.0.payload().unwrap();
            assert!(*flag);
            assert_eq!(handle.lock().unwrap().as_ref(), &vec![1, 2, 3]);
        ...
        #[test]
        fn test_clear_fresh_boxed_tuple_handle_payload() {
            #[cfg(feature = "parallel-rt")]
            for mut list in
                crate::curuntime::allocate_boxed_copperlists::<cumsgs::CuStampedDataSet, 3>()
            {
                assert!(list.msgs.0.0.payload().is_none());
                list.msgs.0.0.clear_payload();
            }
            ...
            {
                let mut lists = crate::curuntime::AsyncCopperListsManager::<
                    cumsgs::CuStampedDataSet,
                    3,
                >::new(None)
                .unwrap();
                for _ in 0..3 {
