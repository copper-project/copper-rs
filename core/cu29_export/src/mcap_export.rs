...
//!
//! Payload schemas are derived from the generated slot metadata and wrapped into
//! the full exported MCAP message envelope (`payload`, `tov`, `process_time`,
//! `status_txt`).
...
///
/// This function uses the slot-aligned output metadata generated by `gen_cumsgs!`
/// to register and trace every concrete payload type. Payload-owned serialized
/// schemas take precedence over reflected field schemas.
///
/// # Arguments
...
where
    ...
    R: Read,
{
    let schemas = payload_schemas_from_output_specs::<P>();
    let task_ids = P::get_all_task_ids();

    export_to_mcap_with_schemas::<P, R>(src, output_path, task_ids, &schemas)
}

fn payload_schemas_from_output_specs<P>() -> Vec<(&'static str, String)>
where
    P: CopperListTuple,
{
    P::get_output_specs()
        ...
        .map(|spec| {
            ...
            spec.register_payload_type(&mut registry);
            let schema = crate::serde_to_jsonschema::registry_to_jsonschema(
                &registry,
                spec.payload_type_path(),
            );
            (spec.task_id, schema)
        })
        .collect()
...
mod tests {
    ...
    use cu29::prelude::{
        ...
        MatchingTasks, Reflect, TaskOutputSpec,
    };
    ...
    impl MatchingTasks for TestMsgs {
        ...
        fn get_output_specs() -> &'static [TaskOutputSpec] {
            const OUTPUT_SPECS: &[TaskOutputSpec] = &[
                TaskOutputSpec::new::<TestPayloadA>("task_a", "TestPayloadA"),
                TaskOutputSpec::new::<TestPayloadB>("task_b", "TestPayloadB"),
            ];
            OUTPUT_SPECS
        }
    }

    impl ErasedCuStampedDataSet for DuplicateTaskMsgs {
        fn cumsgs(&self) -> Vec<&dyn ErasedCuStampedData> {
            vec![&self.0, &self.1]
        }
    }

    impl MatchingTasks for DuplicateTaskMsgs {
        fn get_all_task_ids() -> &'static [&'static str] {
        ...
        fn get_output_specs() -> &'static [TaskOutputSpec] {
            const OUTPUT_SPECS: &[TaskOutputSpec] = &[
                TaskOutputSpec::new::<TestPayloadA>("multi_output", "TestPayloadA"),
                TaskOutputSpec::new::<TestPayloadB>("multi_output", "TestPayloadB"),
            ];
            OUTPUT_SPECS
        }
    }
    ...
    impl MatchingTasks for LargePointCloudMsgs {
        ...
        fn get_output_specs() -> &'static [TaskOutputSpec] {
            const OUTPUT_SPECS: &[TaskOutputSpec] = &[TaskOutputSpec::new::<
                PointCloudSoaHandle<LARGE_POINTS>,
            >(
                "points", "PointCloudSoaHandle"
            )];
            OUTPUT_SPECS
        }
    }
    ...
    fn test_generated_schemas_are_complete() {
        let schemas = payload_schemas_from_output_specs::<TestMsgs>();
    ...
    #[test]
    fn generated_schemas_remain_slot_aligned_for_duplicate_task_ids() {
        let schemas = payload_schemas_from_output_specs::<DuplicateTaskMsgs>();

        assert_eq!(schemas.len(), 2);
        assert_eq!(schemas[0].0, "multi_output");
        assert_eq!(schemas[1].0, "multi_output");
        assert!(schemas[0].1.contains("TestPayloadA"));
