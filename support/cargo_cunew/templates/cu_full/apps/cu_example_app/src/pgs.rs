//! Orchestrates the offline profile-guided scheduling loop.
...
const OUTPUT: &str = "apps/cu_example_app/target/pgs";
const CONTRACT: &str = "apps/cu_example_app/schedule.ron";
const LOGREADER: &str = "cu_example_app-logreader";
const VIEWER: &str = "cu29-view-helper";
...
fn run(action: Action) -> Result<(), String> {
    match action {
        Action::RenderPredictions { candidates } => {
            ...
            fs::write(format!("{OUTPUT}/candidate-count.txt"), candidates.to_string())
        ...
        Action::Select { candidate } => {
            ...
            fs::copy(&source, &selected).map_err(|error| {
                format!("could not select '{}': {error}", source.display())
            })?;
        }
        ...
        Action::Measure {
            ...
            command.args(["run", "-p", "cu_example_app", "--features", "logreader", "--bin", LOGREADER, "--"]);
            ...
            render_view("schedule", Path::new("apps/cu_example_app/copperconfig.ron"),
                Path::new(OUTPUT).join("baseline.measured.schedule.svg"),
                Some(&baseline), Some("baseline"), true)?;
            for candidate in selected {
...
fn candidate_log(candidate: usize) -> PathBuf {
    Path::new("apps/cu_example_app/logs").join(format!("pgs-candidate-{candidate}.copper"))
}

fn parse_candidates(input: &str) -> Result<Vec<usize>, String> {
    let candidates = input
        .split(',')
        .map(|value| value.trim().parse::<usize>().map_err(|error| error.to_string()))
...
fn render_candidate(candidate: usize, log: Option<PathBuf>) -> Result<(), String> {
    ...
    render_view("graph", &config, Path::new(OUTPUT).join(format!("{name}.graph.svg")),
        log.as_deref(), Some(&name), false)?;
    render_view("schedule", &config, Path::new(OUTPUT).join(format!("{name}.schedule.svg")),
        log.as_deref(), Some(&name), false)
}
...
fn render_measured(candidate: usize) -> Result<(), String> {
    ...
    let name = format!("plan-{candidate}");
    render_view("graph", &config, Path::new(OUTPUT).join(format!("{name}.measured.graph.svg")),
        Some(&log), Some(&name), true)?;
    render_view("schedule", &config, Path::new(OUTPUT).join(format!("{name}.measured.schedule.svg")),
        Some(&log), Some(&name), true)
}
...
fn render_view(
    ...
    let mut command = Command::new("cargo");
    command.args(["run", "-p", VIEWER, "--", view, "--app", "cu_example_app", "--config"]);
    command.arg(config).arg("--output").arg(output);
    if let Some(log) = log {
        command.arg("--log").arg(log);
    }
    if view == "schedule" && let Some(candidate) = candidate {
