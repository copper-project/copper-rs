...
- `justfile`: `graph[-log]`, `sched[-log]`, `log`, `cl`, `resim`, and `resim-debug` recipes; `dag` remains as a deprecated graph alias.
{% if pgs_enabled %}
...
`just graph cu_example_app` renders the primary app.
