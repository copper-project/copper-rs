//! Launches the matching Copper graph or schedule viewer for this project.
...
#[derive(Debug, Parser)]
#[command(about = "Render a Copper graph or execution schedule")]
struct Args {
    #[arg(value_enum)]
    view: View,
    ...
    features: Vec<String>,
    /// Copper log base path for observed timing.
    #[arg(long)]
    log: Option<PathBuf>,
    /// PGS candidate name to annotate.
    #[arg(long)]
...
fn run(args: Args) -> Result<(), String> {
    ...
    command
        .arg(&args.config)
        .arg("--output")
        .arg(args.output.unwrap_or_else(|| PathBuf::from(args.view.default_output())));
    if args.open {
        command.arg("--open");
    }
    ...
    if let Some(log) = args.log {
        ...
        command.args(["--logreader", "{{project-name|kebab_case}}-logreader"]);
