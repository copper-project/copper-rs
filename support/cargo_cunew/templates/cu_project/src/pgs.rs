...
const LOGREADER: &str = "{{project-name|kebab_case}}-logreader";
const VIEWER: &str = "{{project-name|kebab_case}}-view";
...
fn run(action: Action) -> Result<(), String> {
    match action {
        ...
        Action::Measure {
            ...
            command.args(["run", "--features", "logreader", "--bin", LOGREADER, "--"]);
            ...
            render_view("schedule", Path::new("copperconfig.ron"),
...
fn render_view(
    ...
    let mut command = Command::new("cargo");
    command.args(["run", "--bin", VIEWER, "--", view, "--config"]);
