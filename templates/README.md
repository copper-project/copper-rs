...
1.  **Change into the new project directory:**
    ...
    The generated workspace includes `apps/`, `tasks/`, and `libs/` so it scales beyond a single crate.
...
The generated project includes helpful utility scripts in its `scripts/` directory:

* `scripts/log`: Use this script to extract and view structured logs generated by your Copper application.
* `scripts/cl`: Use this script to extract CopperLists (sequences of operations) from the application's log output.
* `scripts/rcfg`: Use this script to generate a `.dot` file representing the application's execution Directed Acyclic Graph (DAG). You can use tools like Graphviz to visualize this file (e.g., `dot -Tpng graph.dot -o graph.png`).
